...
describe("Dashboard / Results 聚合语义", () => {
  ...
  it("normalizes legacy retention-only rows into pass while totals remain conserved", () => {
    expect(summarizeResultStatusGroups([
    ...
    ], 0, { passed: { NEEDS_REVIEW: 1, PENDING_RETENTION: 1 } })).toMatchObject({
      total: 6,
      passed: 5,
      review: 1,
    });
  ...
  it("keeps true manual-review evidence separate from retention-only normalization", () => {
    expect(summarizeResultStatusGroups([
      { autoStatus: "PASSED", pageStatus: "NORMAL", _count: { _all: 4 } },
      { autoStatus: "NEEDS_REVIEW", pageStatus: "NORMAL", _count: { _all: 2 } },
    ], 0, {
      passed: { NEEDS_REVIEW: 1 },
      review: { PASSED: 3 },
    })).toMatchObject({
      total: 6,
      passed: 2,
