...
export const PROTECTED_BEHAVIORS = Object.freeze([
  ...
  behavior({ key: "RETENTION_PENDING_NOT_MANUAL_REVIEW", module: "Retention pending workflow", invariant: "当前明确公开且仅留存期限未到必须归为 PENDING_RETENTION，不进入失败原因或人工复核工作量；未知与明确失败保持更高优先级，List/Detail/Summary/Filter/Export 统一，历史结果不改写，到期复查复用唯一 Runner 并以 replacesResultId 幂等创建新结果。", unitTests: ["tests/unit/retention-pending-workflow.test.ts", "tests/unit/audit-retention.test.ts", "tests/unit/audit-result-presentation.test.ts"], e2eTests: ["tests/e2e/historical-extraction-immutable.spec.ts"], unitCases: [caseRef("tests/unit/retention-pending-workflow.test.ts", "Protected RETENTION_PENDING_NOT_MANUAL_REVIEW：留存待到期独立于失败、人工复核、查询和不可变复查"), caseRef("tests/unit/retention-pending-workflow.test.ts", "compares due boundaries by absolute milliseconds"), caseRef("tests/unit/audit-result-presentation.test.ts", "normalizes legacy retention-only NEEDS_REVIEW to PENDING_RETENTION")], e2eCases: [caseRef("tests/e2e/historical-extraction-immutable.spec.ts", "Protected RETENTION_PENDING_NOT_MANUAL_REVIEW：List、Detail、Summary、Filter、Export 与到期复查一致")], fixtures: ["tests/unit/retention-pending-workflow.test.ts", "tests/e2e/historical-extraction-immutable.spec.ts"], triggerFiles: ["lib/audit-engine.ts", "lib/audit-evaluation-consistency.ts", "lib/audit-result-presentation.ts", "lib/automation/queue.ts", "app/api/results", "components/results", "lib/result-query.ts", "lib/result-summary.ts", "lib/import-export-templates/export.ts"] }),
...
export const PROTECTED_BEHAVIOR_GROUPS = Object.freeze({
  ...
  AUDIT_RESULT_PRESENTATION_ALL: Object.freeze(["AUDIT_RESULT_PRESENTATION_IMMUTABLE", "HISTORICAL_EXTRACTION_IMMUTABLE", "RETENTION_PENDING_NOT_MANUAL_REVIEW"]),
});
