...
export const PROTECTED_BEHAVIORS = Object.freeze([
  ...
  behavior({ key: "PAUSE_CONTINUE_RUNNER_HANDOFF", module: "Automation runner lifecycle", invariant: "PROCESSING 中 PAUSE 后立即 CONTINUE 必须由下一代 runner 有界接管；旧 lease 无写权限且后续 Batch 不得饥饿。", unitTests: ["tests/unit/automation-runner-lifecycle.test.ts"], e2eTests: ["tests/e2e/pause-resume-runner-lifecycle.spec.ts"], unitCases: [caseRef("tests/unit/automation-runner-lifecycle.test.ts", "PAUSE 能立即中断等待中的 extraction 而不等待底层 Promise 退出"), caseRef("tests/unit/automation-runner-lifecycle.test.ts", "多次 CONTINUE 使用 generation latch 合并唤醒且不产生双 runner"), caseRef("tests/unit/automation-runner-lifecycle.test.ts", "XHS context close 与下一代 launch 通过 closePromise 串行")], e2eCases: [caseRef("tests/e2e/pause-resume-runner-lifecycle.spec.ts", "Protected PAUSE_CONTINUE_RUNNER_HANDOFF：旧 extraction 延迟退出仍有界接管且后续批次不饥饿")], fixtures: ["tests/e2e/pause-resume-runner-lifecycle.spec.ts"], triggerFiles: ["lib/automation/queue.ts", "lib/automation/runtime-state.ts", "lib/automation/extraction-deadline.ts", "lib/automation/runner-handoff.ts", "lib/automation/browser.ts"] }),
]);
...
export const PROTECTED_BEHAVIOR_GROUPS = Object.freeze({
  ...
  DOUYIN_REGRESSION_ALL: Object.freeze(["DOUYIN_PUBLIC_IMAGE_TEXT_DETAIL"]),
...
export const CHANGE_IMPACT_MAP = Object.freeze([
  ...
  { match: /(?:automation\/queue|automation\/runtime-state|automation\/extraction-deadline|automation\/runner-handoff|pause-resume-runner-lifecycle)/iu, groups: ["AUTOMATION_RUNNER_LIFECYCLE_ALL"], reason: "Runner pause/continue 生命周期变化" },
]);
