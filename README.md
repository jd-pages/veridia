...
账号完全离线运行：开发者使用根目录的 `创建VERIDIA账号.bat` 生成用户名、初始密码和
Ed25519 签名激活码，用户在首次启动页粘贴激活。激活码包含 bcrypt 高成本密码哈希而非
明文密码。登录后的随机会话令牌由 Electron `safeStorage`（Windows DPAPI）加密保存，
数据库只保存 SHA-256。账号创建、重置、续期及纯离线限制见
